...
class MotorControlApp:
    ...
    def create_motor_ui(self, frame):
        motor_configs = [("Motor A", "MA"), ("Motor B", "MB")]
        self.motor_entries = {}

        for i, (label_text, motor_id) in enumerate(motor_configs):
            tk.Label(frame, text=label_text, font=("Arial", 12)).grid(row=i, column=0, padx=5, pady=5, sticky="w")
            entry = tk.Entry(frame, width=10)
            entry.grid(row=i, column=1, padx=5)
            self.motor_entries[motor_id] = entry

            send_command = partial(self.send_motor_command, entry, motor_id)
            ttk.Button(frame, text="Kirim", command=send_command).grid(row=i, column=2, padx=5)
    ...
    def send_motor_command(self, entry, motor_id):
        if not self.ser or not self.ser.is_open:
            return messagebox.showerror("Serial Error", "Port serial tidak tersedia.")
        try:
            pwm = int(entry.get().strip())
            if not -255 <= pwm <= 255:
                raise ValueError("Rentang PWM -255 hingga 255")
            command = f"{motor_id}:{pwm}\n"
            print(f"Mengirim: {command.strip()}")
    ...
    def send_servo_command(self):
        if not self.ser or not self.ser.is_open:
            return messagebox.showerror("Serial Error", "Port serial tidak tersedia.")
        try:
            angle = int(self.servo_entry.get().strip())
            if not 0 <= angle <= 180:
